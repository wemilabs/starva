...
type OrganizationFollowButtonProps = {
...
export function OrganizationFollowButton({
  ...
  const handleFollow = (e: React.MouseEvent) => {
    ...
    startTransition(async () => {
      ...
      if (result.ok) {
        queryClient.invalidateQueries({ queryKey: ["trending-merchants"] });
      } else {
  ...
  if (variant === "compact") {
    return (
      <Button
        ...
        disabled={isPending}
        aria-label={
          optimisticState.isFollowing ? "Unfollow merchant" : "Follow merchant"
        }
